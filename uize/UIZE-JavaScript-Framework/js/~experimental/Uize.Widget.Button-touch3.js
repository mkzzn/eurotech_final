		...
		/*** Private Instance Methods ***/
			...
			_classPrototype._setStateAndFireEvent = function (_domEvent) {
				...
				if (_this.isWired) {
					...
					;
					_this.fire ('>>>>> ' + _domEventType + ' touches = ' + _domEvent.touches.length + ' targetTouches = ' + _domEvent.targetTouches.length);
					if (_domEventType == 'touchstart') _this._isTouch = _true;
						if (_domEvent.touches.length > 1) alert ('HA');
					if (_this._isTouch) {
						if (_domEventType == 'mouseover') return; // ignore mouseover event mapped by mobile Safari
					}
					...
					/*** deferred wiring of other events (for performance) ***/
						if (!_this._allEventsWired) {
							...
							_this.wireNode (
								_this._rootNode,
								_this._isTouch
									? {touchend:_setStateAndFireEvent}
									: {
					...
					if (_isClickable) {
						...
						_this.set ({_state:_isClickEvent && _this._isTouch ? '' : _eventInfo [0]});
