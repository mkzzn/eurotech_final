	...
	builder:function (_superclass) {
		/*** Variables for Scruncher Optimization ***/
			var
				...
				_supportsTouch = _Uize_Node.isSafari // TO DO: come up with a more sophisticated test for touch support
			;
		...
		/*** Private Instance Methods ***/
			_classPrototype._isClickable = function () {
				return (
					this.get ('enabledInherited') && !this.get ('busyInherited') &&
					(!this._selected || this._clickToDeselect)
				);
			};
			...
			_classPrototype._setStateAndFireEvent = function (_domEvent) {
				...
				if (_this.isWired) {
					...
					;
					if (_domEventType == 'touchstart') _this._isTouch = _true;
					if (_this._isTouch && _domEventType.charAt (0) != 't') return; // do we need to prevent default here for link tags?
					...
					/*** deferred wiring of other mouse events (for performance) ***/
						if (!_this._allMouseEventsWired) {
							...
							_supportsTouch && _this.wireNode (_this._rootNode,'touchend',_setStateAndFireEvent);
						}

					if (_isClickEvent) _domEvent.cancelBubble = _true;
					if (_isClickable) {
						var _mouseEventInfo = _mouseEventInfoMap [_domEventType];
						_this.set ({_state:_mouseEventInfo [0]});
						if (_this._isTouch) {
							if (_domEventType == 'touchstart') {
								_this.fire ({name:'Over',domEvent:_domEvent});
								_this.fire ({name:'Down',domEvent:_domEvent});
							} else {
								_this.fire ({name:'Up',domEvent:_domEvent});
								_this.fire ({name:'Click',domEvent:_domEvent});
								_this.fire ({name:'Out',domEvent:_domEvent});
							}
						} else {
							_this.fire ({name:_mouseEventInfo [1],domEvent:_domEvent});
						}
						/*?
							Instance Events
								Click
				...
				if (!_this.isWired) {
					...
					if (_rootNode) {
						...
						/*** wire up event handlers ***/
							...
							_supportsTouch && _this.wireNode (_rootNode,'touchstart',_setStateAndFireEvent);

						_supportsTouch && _this.setNodeStyle (_rootNode,{webkitTapHighlightColor:'rgba(0,0,0,0)'});
							/* NOTE: hide annoying touch highlight in Safari on touch devices (thank you, Ricky)...
					...
					onChange:function () {
						...
						if (!_frameOrder._frameOrderMap) {
							for (
								var
									_frameNo = -1,
									_frameOrderLength = _frameOrder.length,
									_frameOrderMap = _frameOrder._frameOrderMap = {}
								;
								++_frameNo < _frameOrderLength;
							)
								_frameOrderMap [_frameOrder [_frameNo]] = _frameNo
							;
						}
					},
