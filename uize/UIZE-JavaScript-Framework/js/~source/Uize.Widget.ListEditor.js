...
/*?
	...
	In a Nutshell
		...
		Editing the List
			...
			More on Adding Items
				Conforming New Items
					The  =itemConformer= set-get property lets you configure an optional item conformer function to "tidy up" new list items added by the user.
				...
				New Item Placement
					When the user adds a new item, where it shows up in the =list= array is determined by the value of the =sort= set-get property.
		...
		Accessing and Setting the List
			The list that is edited by the user can be accessed programmatically through the =list= set-get property, which is an array of string values.

			The list can also be pre-seeded or updated programmatically by setting the value of this property.

			Value Interface
				The =value= set-get property is registered as an alias to the =list= set-get property.
		...
		List Sorting
			The =sort= set-get property provides a way to maintain the =list= array in a sorted state.

			The list can be sorted ASCIIbetically, in ascending or descending order, or it can be sorted with your own custom sort function, or it can be unsorted with new items added to the beginning or the end of the =list=. When sorting is enabled, the list is kept sorted as items are added by the user, or as its items are otherwise changed - even when items are changed programmatically by setting the value of the =list= set-get property.
		...
		Button State Management
			...
			Disabled Add Button
				When the value of the =input= child widget's =isValid= set-get property is set to =false=, then the =add= button will be disabled.

				This typically occurs when the =input= child widget's value is =''= (an empty string), but could also occur if its value is non-empty but not valid according to a validator function specified by its =validator= set-get property.
								...
								/*?
									Child Widgets
										add
											...
											When the value of the =input= child widget's =isValid= set-get property is set to =false=, then the =add= button will be disabled. This typically occurs when the =input= child widget's value is =''= (an empty string), but could also occur if its value is non-empty but not valid according to a validator function specified by its =validator= set-get property.

											When the =add= button is enabled and is clicked by the user, any text entered in the =input= child widget will first be processed by the =itemConformer= function (if specified) and will then be added to the =list= array. Where the new item appears in the =list= will depend on the value of the =sort= set-get property.
								...
								/*?
									Child Widgets
										remove
											...
											NOTES
											- see the related =list Implied Node= and the =list= set-get property
											- this child widget is added in the constructor
								*/
						...
						/*?
							Implied Nodes
								list Implied Node
									...
									NOTES
									- see the related =list= set-get property
						*/
					);
						...
						Set-get Properties
							list
								...
								Automatically Sorted
									When the value of the =list= property is being set and a value other than ='append'= or ='prepend'= is specified for the =sort= set-get property, then the =list= value will first be conformed by sorting it according to the current sort mode before the new value is applied.
								...
								- see the related =value= set-get property
								- when the list is changed programmatically, the item conformer specified by the =itemConformer= set-get property is not applied to the items in the list - they must already be conformed correctly
								- the initial value is =[]= (an empty array)

							value
								An alias to the =list= set-get property, so that this class effectively implements the =Value Interface=.
					...
					/*?
						Set-get Properties
							sort
								...
								Whenever the value of the =sort= property is changed, the =list= array is resorted according to the new sort mode. Whenever the value of the =list= set-get property is set, it is first conformed by sorting it according to the current sort mode.
