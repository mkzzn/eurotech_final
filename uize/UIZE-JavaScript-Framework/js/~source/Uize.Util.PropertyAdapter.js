...
/*?
	Introduction
		The =Uize.Util.PropertyAdapter= class implements a mechanism for connecting `two set-get properties` together, with optional bi-directional value adapter code.

		*DEVELOPERS:* `Chris van Rensburg`

		In a Nutshell
			The =Uize.Util.PropertyAdapter= class makes it easy to connect `two set-get properties` in a synchronization relationship, where a change in the value of one of the properties results in the value of the other property being synchronized to the new value of the property that changed.

			Any two set-get properties can be connected to each other using a property adapter instance. It can be two properties of the same instance, or it can be properties of two different instances. The properties being connected can belong to widget instances (ie. instances of =Uize.Widget= subclasses), but the instances owning the properties do not have to be widgets. The instances owning the two properties being connected do not even have to be of the same class. And the two properties do not even have to be of the same type or have the same value range - the =Uize.Util.PropertyAdapter= class provides a way to specify a `value adapter` that should be used to adapt the value in either direction.
			...
			A Basic Example
				...
				In the above example, the =minValue= set-get properties of two sliders (instances of the =Uize.Widget.Bar.Slider= class) are being connected through a property adapter, so that if the value of the =minValue= property is modified in =sliderA=, the value of the =minValue= property of =sliderB= is automatically synchronized to that new value, and vice versa.
			...
			Features
				...
				Two Set-get Properties
					Each instance of the =Uize.Util.PropertyAdapter= class can connect two set-get properties together: `property A` and `property B`.

					Property A
						Property A is one of the `two set-get properties` that can be connected together via an instance of the =Uize.Util.PropertyAdapter= class.

						To set the instance and property for `property A`, set the value of the =propertyA= set-get property of the =Uize.Util.PropertyAdapter= instance to a valid `property specifier` (see `A Basic Example`).
					...
					Property B
						Property B is one of the `two set-get properties` that can be connected together via an instance of the =Uize.Util.PropertyAdapter= class.

						To set the instance and property for `property B`, set the value of the =propertyB= set-get property of the =Uize.Util.PropertyAdapter= instance to a valid `property specifier` (see `A Basic Example`).
					...
					Property Specifier
						...
						Property Specifier Object Form
							...
							......................................................................................
							{
								...
								property : propertyNameSTR  // name of a set-get property on the specified instance
							}
							......................................................................................
						...
						Property Specifier Array Form
							...
							[
								...
								propertyNameSTR  // name of a set-get property on the specified instance
							]
							...........................................................................
				...
				Value Adapter
					...
					Use Cases for a Value Adapter
						A value adapter allows the values of the `two set-get properties` that are connected through a property adapter to be encoded or serialized differently, or to have some other form of relationship to one another (such as a mathematical relationship, for example).

						Value Adapter for Mathematical Relationships
							A `value adapter` can be used in the context of a property adapter to maintain a mathematical relationship between `two set-get properties`.
							...
							In the above example, a property adapter is being used to connect the =value= set-get properties of two text input widgets (instances of the =Uize.Widget.TextInput= class): =numberTextInput= and =numberSquaredTextInput=. A value adapter is specified to maintain a relationship where the value of the =numberSquaredTextInput= instance is the square of the value of the =numberTextInput= instance and, conversely, where the value of the =numberTextInput= instance is the square root of the =numberSquaredTextInput= instance.

						Value Adapter for Different Encodings
							A `value adapter` can be used in the context of a property adapter to maintain an encoding relationship between `two set-get properties`.
				...
				Connection and Disconnection
					The `two set-get properties` specified for an instance of the =Uize.Util.PropertyAdapter= class are connected and disconnected under different conditions.

					Initial Connection
						Initial connection is when the `two set-get properties` are either connected for the very first time, or are reconnected after having been disconnected.

						Specifically, initial connection occurs when...

						- the value of the =connected= set-get property becomes =true=, and the value of neither the =propertyA= nor =propertyB= set-get property is =null= or =undefined=

						- immediately after either of `property A` or `property B` are changed or the `value adapter` is changed, and the value of neither the =propertyA= nor =propertyB= set-get property is =null= or =undefined=

						Whenever `initial connection` occurs, the propery adapter instance performs `property A to property B synchronization`, and value change event handlers are wired on the instances that own the `two set-get properties` in order to ensure that the appropriate `synchronization` is performed when the value of either `property A` or `property B` is modified.

					Disconnection
						Disconnection is when `two set-get properties` that were previously connected through a property adapter instance become disconnected so that their values are no longer kept synchronized.

						Specifically, disconnection occurs when two set-get properties were previously connected and then ...

						- the value of the =connected= set-get property becomes =false=

						- the value of any of the =propertyA=, =propertyB=, or =valueAdapter= set-get properties is changed (if the value of neither the =propertyA= nor =propertyB= set-get property is =null= or =undefined= after the change, then disconnection will occur right before the `initial connection` that occurs for the new configuration of connected properties and/or `value adapter`)

				Synchronization
					Synchronization is when the value of one of the `two set-get properties` that are connected through a property adapter instance changes, and the value of the other property in the pair is updated in order to keep it synchronized with the one that changed.

					When `two set-get properties` are specified for a property adapter instance, but that instance's =connected= set-get property is set to =false=, then synchronization will not be performed if the value of either of the `two set-get properties` is modified. Whenever synchronization is performed, the value of the =valueAdapter= set-get property is taken into consideration (see `How a Value Adapter is Used`).

					Property A to Property B Synchronization
						Once `initial connection` has occured between `two set-get properties` specified for a property adapter instance, a change in the value of `property A` will result in the value of `property B` being synchronized to the new value of `property A`.
						...
						Property A to Property B Synchronization on Initial Connection
							Unlike `property B to property A synchronization`, synchronization from property A to property B is also performed upon `initial connection` of the `two set-get properties`.
					...
					Property B to Property A Synchronization
						Once `initial connection` has occured between `two set-get properties` specified for a property adapter instance, a change in the value of `property B` will result in the value of `property A` being synchronized to the new value of `property B`.
				...
				Dynamically Updatable
					The =Uize.Util.PropertyAdapter= class supports dynamic updating of all of its set-get properties.
					...
					Dynamically Connect and Disconnect
						The =Uize.Util.PropertyAdapter= class supports dynamically connecting and disconnecting the `two set-get properties` that are connected through a property adapter instance.

						The properties can be connected or disconneced at any time, simply by setting the value of the property adapter instance's =connected= set-get property.

					Dynamically Change the Properties
						Any or both of the `two set-get properties` that are connected through a property adapter instance can be changed midstream, simply by setting the value of either or both of the property adapter instance's =propertyA= and =propertyB= set-get properties.
				...
				Uize.Util.PropertyAdapter vs Uize.Util.Coupler
					...
					- the =Uize.Util.PropertyAdapter= class supports connecting any two set-get properties of any two instances (eg. the =textColor= property of some instance with the =bgColor= property of some other instance), while the =Uize.Util.Coupler= class supports connecting the same set of any number of set-get properties across two or more instances (eg. the =minValue= and =maxValue= properties of three different slider widget instances)
			...
			_class.registerProperties ({
				_connected:{
					...
					/*?
						Set-get Properties
							connected
								A boolean, indicating whether or not the connection is active and the properties specified by the =propertyA= and =propertyB= set-get properties are connected together.
					...
					/*?
						Set-get Properties
							propertyA
								...
								NOTES
								- see the companion =propertyB= set-get property
					...
					/*?
						Set-get Properties
							propertyB
								...
								NOTES
								- see the companion =propertyA= set-get property
								- the initial value is =undefined=
					*/
				},
				_valueAdapter:{
					...
					/*?
						Set-get Properties
							valueAdapter
								...
								NOTES
								- see the related =propertyA= and =propertyB= set-get properties
								- the initial value is =undefined=
					*/
				}
			});
